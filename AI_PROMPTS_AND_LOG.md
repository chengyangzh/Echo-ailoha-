...
**处理结果**：最终把 Echo 定义成了一个“search by structure, not by words”的结构类比 agent。Runtime 只规定合法动作和少数不变量，LLM 自己选择 direct answer、`search`、`read_case`、`analogy_board` 或其他工具；新的 concrete situation 默认被解释成 implicit analogy request，这样 Echo 不会退化成普通聊天助手。这个决定也确定了后面所有功能的优先级：复杂度应该集中在结构抽象、候选检索、证据检查和跨轮状态，不是泛化成一个无边界的 agent 平台。
